...
void QuakeApp::OnGlobeBtnPressed() {
    ...
    if(rep==false){
        earth_.SwitchToGlobe();
        rep = true;
    }else{
        earth_.SwitchToPlane();
        rep = false;
    }
}
...
void QuakeApp::DrawUsingOpenGL() {
    ...
    float range = quake_db_.max_magnitude()-quake_db_.min_magnitude();
    for(int i = quake_db_.min_index(); i <= quake_db_.max_index(); i++){
        Date d = Date(current_time_);
        Date minDate = Date(d.month(), d.day(), d.year()-1, d.hour(), d.minute(), d.second());
        if(quake_db_.earthquake(i).date()>minDate&& quake_db_.earthquake(i).date()<d){
            float redVal = quake_db_.earthquake(i).magnitude()-quake_db_.min_magnitude();


            Color c(redVal/range, .5, .2);
            Point3 mp;
            if(rep == false){
                mp = earth_.LatLongToPlane(quake_db_.earthquake(i).latitude(), quake_db_.earthquake(i).longitude());
            }else{
                mp = earth_.LatLongToSphere(quake_db_.earthquake(i).latitude(), quake_db_.earthquake(i).longitude());
            }
            std::vector<Point3> points;
            points.push_back(Point3(mp[0],mp[1]+.02,0));
            points.push_back(Point3(mp[0]+.01,mp[1]+.01,0));
            points.push_back(Point3(mp[0]-.01,mp[1]-.01,0));
            points.push_back(Point3(mp[0],mp[1]-.02,0));

            quick_shapes_.DrawLines(model_matrix, view_matrix_, proj_matrix_, c, points, QuickShapes::LinesType::LINE_STRIP, .1);
        }
    }
