                           ...
                           const std::vector<Point2> &stroke2d)
{
    ...
    Vector3 SE = E-S;
    Vector3 normToPlane = SE.Cross(Vector3(0,1,0));
    ...
    for(int i = 0; i <stroke2d.size(); i++){
        ...
        Ray ray(eye, (pt3d - eye).ToUnit());
        ray.IntersectPlane(S, normToPlane, &t, &x);
        silhouette_curve.push_back(x);
    }
    ...
    for (int i=0; i<ground_mesh_.num_vertices(); i++) {
        ...
        Point3 Pproj = P.ClosestPointOnPlane(S, normToPlane);
        
        float h = hfunc(normToPlane, silhouette_curve, Pproj);
        Point3 Pnew;
        if (h!=0){
            float d = P.DistanceToPlane(S, normToPlane);
            float wd = 1-(d/5)*(d/5);
            if(wd < 0){
                wd = 0;
            }
            float Pynew =(1-wd)*P.y()+wd*h;
            Pnew = Point3(P.x(), Pynew, P.z());
        }else{
            Pnew = P;
        }
        P = Pnew;
