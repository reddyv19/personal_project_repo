// Election.java
// This file contains the Electoin object. It determines the outcome of both an IR and OPL election.
...
public class Election {
    ...
    private void updateAudit(int round) throws IOException {
        ...
        //IR election
            ...
            if (round == 1){
                ...
                //print the total amount of ovotes
                pw.printf("Total Votes: %d %n",totalVotes);
            }
    ...
    private void runIR(){
        ...
        for (int i = 0; i < totalVotes; i++){
            IRBallot temp =  (IRBallot) ballots[i];
            ...
            for (int i = 0; i < totalCandidates; i++) {
                if (!candidatesArray[i].isEliminated()) {
                    if (minCandidate != -1) {
                        ...
                        if (candidatesArray[minCandidate].isGreaterThan(candidatesArray[i], false)) {
                            minCandidate = i;
                        }
        ...
        assert can1 != null;
        assert can2 != null;
        if (can1.isGreaterThan(can2,false)){
            can2.eliminate();
        } else {
            can1.eliminate();
        }
    ...
    private void runOPL() {
        ...
        //give candidates their votes and ballots
        for (int i = 0; i < totalVotes; i++){
            OPLBallot temp =  (OPLBallot) ballots[i];
        ...
        while(seatsRemain > 0){
            ...
            for (int i = 1; i < partyArray.size(); i++){
                if (partyArray.get(i).greaterRemain(partyArray.get(maxRemainder),false)){
                    maxRemainder = i;
                }
            }
    ...
    private void IRInit(){
        ...
        //Intitalizes all of the Candidates
        fileReader.nextLine();
        ...
        totalVotes = fileReader.nextInt();
        fileReader.nextLine();
        
        //Initializes all of the Ballots
        ballots = new IRBallot[totalVotes];
        for (int i = 0; i < totalVotes; i++) {
            String rankings = fileReader.nextLine();
            ballots[i] = new IRBallot(rankings, i);
        }
    }
    ...
    private void OPLInit() {
        ...
        //Initializes all of the ballots and stres them in the ballot array
        ballots = new OPLBallot[totalVotes];
        for (int i = 0; i < totalVotes; i++) {
            String rankings = fileReader.nextLine();
            ballots[i] = new OPLBallot(rankings, i);
        }
    }
